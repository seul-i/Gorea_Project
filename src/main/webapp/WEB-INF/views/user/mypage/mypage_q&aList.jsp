	...
	<div class="menuContainer">
		<div class="menuinner">
			<div class="mypage-menuBtn">
				<a href="userMypage.do" class="mypagebtn active userInfo"> <c:choose>
				...
				</a> <a href="userBoardList.do?userSeq=${userSeq}"
					class="mypagebtn active boardList"> <c:choose>
				...
				</a> <a href="userReplyList.do?userSeq=${userSeq}"
					class="mypagebtn active replyList"> <c:choose>
				...
				</a> <a href="#" class="mypagebtn active likeList"> <c:choose>
				...
				</a> <a href="userQnaList.do?userSeq=${userSeq}"
					class="mypagebtn active qna"> <c:choose>
				...
				</a> <a href="userDropAccount.do?userSeq=${userSeq}"
					class="mypagebtn active bye"> <c:choose>
						...
						<c:when test="${not empty lists}">
							<c:forEach var="qna" items="${lists}">
								<tbody>
									<tr class="boardList">
										<td><c:choose>
												<c:when test="${qna.qnaCmt == 0}">답변 대기중</c:when>
												<c:otherwise>답변완료</c:otherwise>
											</c:choose></td>
										<td><a
											href="/${language}/qna_view.do?qnaSeq=${qna.qnaSeq}">[${qna.qnaCategory}]${qna.qnaTitle}</a>
										</td>
				...
				<div class="pagination-container">
					<div class="pagination">
						...
						<c:if test="${paging.cpage > 1}">
							<a href="/user/${language}/qnaList.do?userSeq=${userSeq }cpage=1"
								class="pagination-item">&lt;&lt;</a>
							<a
								href="/user/${language}/qnaList.do?userSeq=${userSeq }&cpage=${paging.cpage - 1}"
								class="pagination-item">&lt;</a>
						</c:if>
						<c:if test="${paging.cpage == 1}">
						...
						<c:forEach var="i" begin="${paging.firstPage}"
							...
							<c:choose>
								...
								<c:otherwise>
									<a
										href="/user/${language}/qnaList.do?userSeq=${userSeq}&cpage=${i}"
						...
						<c:if test="${paging.cpage < paging.totalPage}">
							<a
								...
								class="pagination-item">&gt;</a>
							<a
								href="/user/${language}/qnaList.do?userSeq=${userSeq}&cpage=${paging.totalPage}"
								class="pagination-item">&gt;&gt;</a>
						</c:if>
						<c:if test="${paging.cpage == paging.totalPage}">
							<span class="pagination-item disabled">&gt;</span>
