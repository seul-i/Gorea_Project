...
<script type="text/javascript">
    ...
    function goToPreviousPage() {
        ...
        if (currentPage > 1) {
            // 이동할 URL 생성
            var url = '/'+language+'/editTip_list.do?cpage=' + (currentPage - 1);
    ...
    $(document).ready(function () {
        ...
        $('.album').on('click', function () {
        	...
        	if (!("${not empty SPRING_SECURITY_CONTEXT}" == "true")) {
	            ...
	            window.location.href = '/'+language+'/editTip_view.do?edittipSeq=' + seq;
...
<body>
	...
	<div class="commonBanner" id="comBanner">
        <img src="/img/banner/editreTipbanner.jpg" alt="banner" style="object-position: center;">
        
        <div class="commonBanner-text">
            <h1>
            <c:choose>
                <c:when test="${language eq 'korean'}">에디터 꿀팁</c:when>
                <c:when test="${language eq 'english'}">Editor Travel Tips</c:when>
                <c:when test="${language eq 'japanese'}">エディター旅行のヒント</c:when>
                <c:when test="${language eq 'chinese'}">编辑旅游小贴士</c:when>
            </c:choose>
            </h1>
        </div>
	</div>
	...
	<div class="location">
		...
		<c:choose>
			<c:when test="${language eq 'korean'}">
				...
				<span> <a href="./editTip_list.do">에디터 꿀팁</a>
				</span>
			...
			<c:when test="${language eq 'english'}">
				...
				<span> <a href="./editTip_list.do">Editor Travel
						Tips</a>
				</span>
			...
			<c:choose>
				<c:when test="${not empty lists1}">
					<c:forEach var="eto" items="${lists1}">
						<div class='album' data-seq='${eto.edittipSeq}'>
							<div class='image'>
								<c:choose>
									<c:when test="${eto.firstImageUrl.startsWith('https://')}">
										<img src='${eto.firstImageUrl}' alt='' />
									</c:when>
								</c:choose>
				...
				<c:otherwise>
					<c:choose>
						...
						<c:when test="${language eq 'japanese'}">
							<div class="noData">データなし</div>
						</c:when>
						<c:when test="${language eq 'chinese'}">
							<div class="noData">无可用数据</div>
						</c:when>
					</c:choose>
				</c:otherwise>
			</c:choose>
	...
	<c:choose>
		<c:when test="${role eq 'ROLE_ADMIN'}">
			<div class="write_button_container">
				<a href="/korean/editTip_write.do" class="write_button">글쓰기</a>
			</div>
		</c:when>
	</c:choose>
	...
	<div class="pagination-container">
		<div class="pagination">
			...
			<c:if test="${paging.cpage > 1}">
				<a
					href="/${language}/editTip_list.do?cpage=1<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
					class="pagination-item">&lt;&lt;</a>
				<a
					href="/${language}/editTip_list.do?cpage=${paging.cpage - 1}<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
					class="pagination-item">&lt;</a>
			</c:if>
			<c:if test="${paging.cpage == 1}">
			...
			<c:forEach var="i" begin="${paging.firstPage}"
				...
				<c:choose>
					...
					<c:otherwise>
						<a
							href="/${language}/editTip_list.do?cpage=${i}<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
			...
			<c:if test="${paging.cpage < paging.totalPage}">
				<a
					...
					class="pagination-item">&gt;</a>
				<a
					href="/${language}/editTip_list.do?cpage=${paging.totalPage}<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
					class="pagination-item">&gt;&gt;</a>
			</c:if>
			<c:if test="${paging.cpage == paging.totalPage}">
				<span class="pagination-item disabled">&gt;</span>
