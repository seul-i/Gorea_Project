	...
	<div class="main-container">
		<div class="container search-container">
			<form action="/${language}/totalsearch.do" method="get">
				<div class="searchs-box">
					<input type="text" name="keyword" class="searchs" required
						placeholder="키워드를 입력해 주세요">
					<button type="submit" class="searchBtns">검색</button>
				</div>
			</form>
			<h3 style="text-align: center;">'${param.keyword}'의 검색 결과 입니다.
				(${totalCount}건)</h3>
		</div>
		...
		<div class="container board-counts">
			...
			<div class="board-count">
				<a
					href="/${language}/totalsearch.do?boardType=AllBoards&keyword=${param.keyword}">전체
					게시판 (${totalCount})</a>
			</div>
			<div class="board-count">
				<a
        ...
        document.querySelectorAll('.content-wrap span').forEach(
            function(span) {
                if (span.textContent.length > 50) {
                    span.textContent = span.textContent.substring(0, 50) + '...';
                }
            });
    ...
    <div class="pagination-container">
        <div class="pagination">
            ...
            <c:if test="${paging.cpage > 1}">
                <a href="/${language}/totalsearch.do?boardType=${currentTab}&keyword=${param.keyword}&cpage=1" class="pagination-item">&lt;&lt;</a>
                <a href="/${language}/totalsearch.do?boardType=${currentTab}&keyword=${param.keyword}&cpage=${paging.cpage - 1}" class="pagination-item">&lt;</a>
            </c:if>
            <c:if test="${paging.cpage == 1}">
            ...
            <c:forEach var="i" begin="${paging.firstPage}" end="${paging.lastPage}" varStatus="loop">
                <c:choose>
                    ...
                    <c:otherwise>
                        <a href="/${language}/totalsearch.do?boardType=${currentTab}&keyword=${param.keyword}&cpage=${i}" class="pagination-item">${i}</a>
                    </c:otherwise>
                </c:choose>
            </c:forEach>
            ...
            <c:if test="${paging.cpage < paging.totalPage}">
                <a href="/${language}/totalsearch.do?boardType=${currentTab}&keyword=${param.keyword}&cpage=${paging.cpage + 1}" class="pagination-item">&gt;</a>
                <a href="/${language}/totalsearch.do?boardType=${currentTab}&keyword=${param.keyword}&cpage=${paging.totalPage}" class="pagination-item">&gt;&gt;</a>
            </c:if>
            <c:if test="${paging.cpage == paging.totalPage}">
                <span class="pagination-item disabled">&gt;</span>
                <span class="pagination-item disabled">&gt;&gt;</span>
