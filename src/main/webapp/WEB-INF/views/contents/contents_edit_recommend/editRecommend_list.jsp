...
<script type="text/javascript">
        ...
        function goToPreviousPage() {
            ...
            if (currentPage > 1) {
                // 이동할 URL 생성
                var url = '/'+language+'/editRecommend_list.do?cpage=' + (currentPage - 1);
						...
						<div class='album' data-seq='${to.editrecoSeq}'>
							...
							<div class='content'>
								<div class='title-subtitle'>
									<div class='title'>
										<a
											href='./editRecommend_view.do?editrecoSeq=${to.editrecoSeq}<c:if test="${not empty param.cpage}">&cpage=${param.cpage}</c:if><c:if test="${not empty param.searchType}">&searchType=${param.searchType}</c:if><c:if test="${not empty param.searchKeyword}">&searchKeyword=${param.searchKeyword}</c:if>'>
											${to.editrecoSubject} </a>
									</div>
	...
	<c:choose>
		<c:when test="${role eq 'ROLE_ADMIN'}">
			<div class="write_button_container">
				<a href="/korean/editRecommend_write.do" class="write_button">글쓰기</a>
			</div>
		</c:when>
	</c:choose>
	...
	<div class="pagination-container">
		<div class="pagination">
			...
			<c:if test="${paging.cpage > 1}">
				<a
					href="/${language}/editRecommend_list.do?cpage=1<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
					class="pagination-item">&lt;&lt;</a>
				<a
					href="/${language}/editRecommend_list.do?cpage=${paging.cpage - 1}<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
					class="pagination-item">&lt;</a>
			</c:if>
			<c:if test="${paging.cpage == 1}">
			...
			<c:forEach var="i" begin="${paging.firstPage}"
				...
				<c:choose>
					...
					<c:otherwise>
						<a
							href="/${language}/editRecommend_list.do?cpage=${i}<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
			...
			<c:if test="${paging.cpage < paging.totalPage}">
				<a
					...
					class="pagination-item">&gt;</a>
				<a
					href="/${language}/editRecommend_list.do?cpage=${paging.totalPage}<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
					class="pagination-item">&gt;&gt;</a>
			</c:if>
			<c:if test="${paging.cpage == paging.totalPage}">
				<span class="pagination-item disabled">&gt;</span>
