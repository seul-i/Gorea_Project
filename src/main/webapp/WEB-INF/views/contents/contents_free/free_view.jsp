        ...
        <div class="post-info">
        	<c:choose>
            	...
            	<c:when test="${language eq 'english'}">
		            <div class="post-info-left">
		                <span class="post-info-item">Writer: ${to.userNickname }</span>
		            </div>
		            <div class="post-info-right">
		                <span class="post-info-item">PostDate: <c:out value="${to.freepostDate}"/></span>
		                <span class="post-info-item">Views: <c:out value="${to.freeHit}"/></span>
		            </div>
            	</c:when>
            	<c:when test="${language eq 'japanese'}">
		            <div class="post-info-left">
		                <span class="post-info-item">執筆: ${to.userNickname }</span>
		            </div>
		            <div class="post-info-right">
		                <span class="post-info-item">日付: <c:out value="${to.freepostDate}"/></span>
		                <span class="post-info-item">ビュー: <c:out value="${to.freeHit}"/></span>
		            </div>
            	</c:when>
            	<c:when test="${language eq 'chinese'}">
		            <div class="post-info-left">
		                <span class="post-info-item">作家: ${to.userNickname }</span>
		            </div>
		            <div class="post-info-right">
		                <span class="post-info-item">日期: <c:out value="${to.freepostDate}"/></span>
		                <span class="post-info-item">查看: <c:out value="${to.freeHit}"/></span>
		            </div>
            	</c:when>
            </c:choose>
        </div>
       ...
       <div class="post-actions">
       		<c:choose>
			    ...
			    <c:when test="${language eq 'japanese' }">
				    ...
				    <div class="left-buttons">
				        <c:if test="${not empty prevPost}">
				            <input type="button" value="Pre" class="btn" onclick="location.href='freeboard_view.do?freeSeq=${prevPost.freeSeq}&cpage=${param.cpage}&searchType=${param.searchType}&searchKeyword=${fn:escapeXml(param.searchKeyword)}'" />
				        </c:if>
				        <c:if test="${not empty nextPost}">
				            <input type="button" value="Next" class="btn" onclick="location.href='freeboard_view.do?freeSeq=${nextPost.freeSeq}&cpage=${param.cpage}&searchType=${param.searchType}&searchKeyword=${fn:escapeXml(param.searchKeyword)}'" />
				        </c:if>
				    </div>
			   		<!-- 삭제, 수정, 목록 버튼 그룹 -->
				    <div class="right-buttons">
