...
<body>
    ...
    <main>
        ...
        <div class="containers">
            ...
            <div class="post-title"><c:out value="${to.editrecoSubject}" /></div>
            <div class="post-info">
                <div class="post-info-left">
                    <span class="post-info-item">작성자: 관리자</span>
                    <span class="post-info-item">작성일: <c:out value="${to.editrecoWdate}" /></span>
                    <span class="post-info-item">조회수: <c:out value="${to.editrecoHit}" /></span>
                </div>
            </div>
            <div class="post-content">
            	<img src='${to.firstImageUrl}' alt=''>
                <c:out value="${to.editrecoContent}" escapeXml="false" />

            </div>
			<c:url var="deleteUrl" value="/adminEditReco_delete_ok.do">
            <c:param name="editrecoSeq" value="${seq}" />
        ...
        <c:url var="modifyUrl" value="/adminEditReco_modify.do">
            <c:param name="editrecoSeq" value="${seq}" />
            <c:if test="${not empty param.cpage}"><c:param name="cpage" value="${param.cpage}" /></c:if>
            <c:if test="${not empty param.searchType}"><c:param name="searchType" value="${param.searchType}" /></c:if>
            <c:if test="${not empty param.searchKeyword}"><c:param name="searchKeyword" value="${param.searchKeyword}" /></c:if>
        ...
        <c:url var="listUrl" value="/adminEditReco.do">
            <c:if test="${not empty param.cpage}"><c:param name="cpage" value="${param.cpage}" /></c:if>
            <c:if test="${not empty param.searchType}"><c:param name="searchType" value="${param.searchType}" /></c:if>
            <c:if test="${not empty param.searchKeyword}"><c:param name="searchKeyword" value="${param.searchKeyword}" /></c:if>
        </c:url>
         ...
         <div class="post-actions">
                <div class="left-buttons">
	         		<c:if test="${not empty prevPost}">
			            <input type="button" value="이전글" class="btn" onclick="location.href='adminEditreco_view.do?editrecoSeq=${prevPost.editrecoSeq}&cpage=${param.cpage}&searchType=${param.searchType}&searchKeyword=${fn:escapeXml(param.searchKeyword)}'" />
			        </c:if>
			        <c:if test="${not empty nextPost}">
			            <input type="button" value="다음글" class="btn" onclick="location.href='adminEditreco_view.do?editrecoSeq=${nextPost.editrecoSeq}&cpage=${param.cpage}&searchType=${param.searchType}&searchKeyword=${fn:escapeXml(param.searchKeyword)}'" />
			        </c:if>
   		 		</div>
