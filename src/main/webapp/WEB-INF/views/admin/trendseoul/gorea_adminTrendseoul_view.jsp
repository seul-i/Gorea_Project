...
<body>
    ...
    <main>
        ...
        <div class="containers">
            ...
            <div class="post-title"><c:out value="${to.seoulTitle}" /></div>
            <div class="post-info">
                <div class="post-info-left">
                    <span class="post-info-item">작성자: 관리자</span>
                    <span class="post-info-item">작성일: <c:out value="${to.seoulpostDate}" /></span>
                    <span class="post-info-item">조회수: <c:out value="${to.seoulHit}" /></span>
                </div>
            </div>
            <div class="post-content">
            	<img src='${to.firstImageUrl}' alt=''>
                <c:out value="${to.seoulContent}" escapeXml="false" />
                
                <!--  
                <c:out value="${to.seoulLoc}" />
                <c:out value="${to.seoulSite}" />
                <c:out value="${to.seoulusageTime}" />
                <c:out value="${to.seoulusageFee}" />
                <c:out value="${to.seoulTrafficinfo}" />
                <c:out value="${to.seoulNotice}" /> -->

            </div>
			<c:url var="deleteUrl" value="/adminTrend_delete_ok.do">
            <c:param name="seoulSeq" value="${seq}" />
        ...
        <c:url var="modifyUrl" value="/adminTrend_modify.do">
            <c:param name="seoulSeq" value="${seq}" />
            <c:if test="${not empty param.cpage}"><c:param name="cpage" value="${param.cpage}" /></c:if>
            <c:if test="${not empty param.searchType}"><c:param name="searchType" value="${param.searchType}" /></c:if>
            <c:if test="${not empty param.searchKeyword}"><c:param name="searchKeyword" value="${param.searchKeyword}" /></c:if>
        ...
        <c:url var="listUrl" value="/adminTrendseoul.do">
            <c:if test="${not empty param.cpage}"><c:param name="cpage" value="${param.cpage}" /></c:if>
            <c:if test="${not empty param.searchType}"><c:param name="searchType" value="${param.searchType}" /></c:if>
            <c:if test="${not empty param.searchKeyword}"><c:param name="searchKeyword" value="${param.searchKeyword}" /></c:if>
        </c:url>
         ...
         <div class="post-actions">
	         ...
	         <c:if test="${not empty prevPost}">
	            <input type="button" value="이전글" class="btn" onclick="location.href='adminTrendseoul_view.do?seoulSeq=${prevPost.seoulSeq}&cpage=${param.cpage}&searchType=${param.searchType}&searchKeyword=${fn:escapeXml(param.searchKeyword)}'" />
	        </c:if>
	        <c:if test="${not empty nextPost}">
	            <input type="button" value="다음글" class="btn" onclick="location.href='adminTrendseoul_view.do?seoulSeq=${nextPost.seoulSeq}&cpage=${param.cpage}&searchType=${param.searchType}&searchKeyword=${fn:escapeXml(param.searchKeyword)}'" />
	        </c:if>
   		 </div>
