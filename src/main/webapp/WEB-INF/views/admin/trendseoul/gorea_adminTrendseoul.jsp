			...
			<div class="board_list_wrap">
				<div class="board_list">
					...
					<c:if test="${not empty lists}">
						<c:forEach items="${lists}" var="to">
							<div class="list-item">
								<div class='num'>${to.seoulSeq}</div>
								<div class='title'>
									    <a href='./adminTrendseoul_view.do?seoulSeq=${to.seoulSeq}<c:if test="${not empty param.cpage}">&cpage=${param.cpage}</c:if><c:if test="${not empty param.searchType}">&searchType=${param.searchType}</c:if><c:if test="${not empty param.searchKeyword}">&searchKeyword=${param.searchKeyword}</c:if>'>
									    ${to.seoulTitle}
									</a>
								</div>
								<div class='writer'>관리자</div>
								<div class='date'>${to.seoulpostDate}</div>
								<div class='count'>
									<a href='adminTrend_delete_ok.do?seoulSeq=${to.seoulSeq}'
				...
				<div class="bottom-container">
					<div class="pagination-container">
						<div class="pagination">
							...
							<c:if test="${paging.cpage > 1}">
								<a
									href="adminTrendseoul.do?cpage=1<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
									class="pagination-item">&lt;&lt;</a>
								<a
									href="adminTrendseoul.do?cpage=${paging.cpage - 1}<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
									class="pagination-item">&lt;</a>
							</c:if>
							<c:if test="${paging.cpage == 1}">
							...
							<c:forEach var="i" begin="${paging.firstPage}"
								...
								<c:choose>
									...
									<c:otherwise>
										<a
											href="adminTrendseoul.do?cpage=${i}<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
							...
							<c:if test="${paging.cpage < paging.totalPage}">
								<a
									...
									class="pagination-item">&gt;</a>
								<a
									href="adminTrendseoul.do?cpage=${paging.totalPage}<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
									class="pagination-item">&gt;&gt;</a>
							</c:if>
							<c:if test="${paging.cpage == paging.totalPage}">
								<span class="pagination-item disabled">&gt;</span>
					...
					<div class="write_button_container">
                                <a href="adminTrend_write.do" class="write_button">글쓰기</a>
                            </div>
				</div>

				<!-- 검색 부분 -->
				<div class="search_container">
					<form action="adminTrendseoul.do" method="get">
