	...
	<main>
		...
		<div class="containers">
			...
			<div class="post-title">
				<c:out value="${to.noticeTitle}" />
			</div>
			<div class="post-info">
				<div class="post-info-left">
					<span class="post-info-item">작성자: 관리자</span> <span
						class="post-info-item">작성일: <c:out
							value="${to.noticepostDate}" /></span> <span class="post-info-item">조회수:
						<c:out value="${to.noticeHit}" />
					</span>
				</div>
			</div>
			...
			<c:url var="deleteUrl" value="/adminnotice_delete_ok.do">
			...
			<c:url var="modifyUrl" value="/adminnotice_modify.do">
				<c:param name="noticeSeq" value="${seq}" />
				<c:if test="${not empty param.cpage}">
					<c:param name="cpage" value="${param.cpage}" />
				</c:if>
			...
			<c:url var="listUrl" value="/adminnotice.do">
				<c:if test="${not empty param.cpage}">
					<c:param name="cpage" value="${param.cpage}" />
				</c:if>
				<c:if test="${not empty param.searchType}">
			...
			<div class="post-actions">
				<div class="left-buttons">
					<c:if test="${not empty prevPost}">
						<input type="button" value="이전글" class="btn"
							onclick="location.href='adminnotice_view.do?noticeSeq=${prevPost.noticeSeq}&cpage=${param.cpage}&searchType=${param.searchType}&searchKeyword=${fn:escapeXml(param.searchKeyword)}'" />
					</c:if>
					<c:if test="${not empty nextPost}">
						<input type="button" value="다음글" class="btn"
							onclick="location.href='adminnotice_view.do?noticeSeq=${nextPost.noticeSeq}&cpage=${param.cpage}&searchType=${param.searchType}&searchKeyword=${fn:escapeXml(param.searchKeyword)}'" />
					</c:if>
				</div>
				<div class="right-buttons">
					<input type="button" value="삭제" class="btn"
						onclick="confirmDelete('${deleteUrl}')" /> <input type="button"
						value="수정" class="btn" onclick="location.href='${modifyUrl}'" />
