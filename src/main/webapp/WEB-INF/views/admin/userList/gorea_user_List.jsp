...
<body>
	...
	<main>
		...
		<c:set var="userList" value="${paging.userList}" />
		<c:set var="role"
			...
			<div class="board_list_wrap">
				<div class="board_list">
					<div class="top">
						...
						<div class="userFirstname">이름</div>
						<div class="userFirstname">닉네임</div>
						<div class="userNation">국적</div>
					...
					<c:if test="${not empty lists}">
						<c:forEach items="${lists}" var="to">
							<div class="list-item">
								<div class='num'>${to.userSeq}</div>
								<div class="username">${to.username }</div>
								<div class="userFirstname">${to.userFirstname }</div>
								<div class='userNickname'>${to.userNickname }</div>
								<div class="userNation">${to.userNation }</div>
								<div class='date'>${to.userJoindate}</div>
								<div class='count'>
									<a href='adminUser_delete_ok.do?userSeq=${to.userSeq}'
					...
					<div class="pagination-container">
						<div class="pagination">
							...
							<c:if test="${paging.cpage > 1}">
								<a
									href="adminUserList.do?cpage=1<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
									class="pagination-item">&lt;&lt;</a>
								<a
									href="adminUserList.do?cpage=${paging.cpage - 1}<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
									class="pagination-item">&lt;</a>
							</c:if>
							<c:if test="${paging.cpage == 1}">
							...
							<c:forEach var="i" begin="${paging.firstPage}"
								...
								<c:choose>
									...
									<c:otherwise>
										<a
											href="adminUserList.do?cpage=${i}<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
							...
							<c:if test="${paging.cpage < paging.totalPage}">
								<a
									...
									class="pagination-item">&gt;</a>
								<a
									href="adminUserList.do?cpage=${paging.totalPage}<c:if test="${not empty param.searchType and not empty param.searchKeyword}">&searchType=${param.searchType}&searchKeyword=${param.searchKeyword}</c:if>"
									class="pagination-item">&gt;&gt;</a>
							</c:if>
							<c:if test="${paging.cpage == paging.totalPage}">
								<span class="pagination-item disabled">&gt;</span>
				...
				<div class="search_container">
					<form action="adminUserList.do" method="get">
						<select name="searchType">
							...
							<option value="userFirstname">이름</option>
