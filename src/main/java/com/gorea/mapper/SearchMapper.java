...
public interface SearchMapper {
	...
	// 자유게시판 검색
    @Select("SELECT 'FreeBoard' AS boardType, freeSeq AS id, freeTitle AS title, freeContent AS content, date_format(freepostDate, '%Y-%m-%d') AS postDate, freeHit AS hit FROM Freeboard WHERE freeTitle LIKE CONCAT('%', #{keyword}, '%') OR freeContent LIKE CONCAT('%', #{keyword}, '%')")
    List<Gorea_SearchResultTO> searchFreeBoard(@Param("keyword") String keyword);

    // 공지사항 검색
    @Select("SELECT 'Notice' AS boardType, noticeSeq AS id, noticeTitle AS title, noticeContent AS content, date_format(noticepostDate, '%Y-%m-%d') AS postDate, noticeHit AS hit FROM Notice WHERE noticeTitle LIKE CONCAT('%', #{keyword}, '%') OR noticeContent LIKE CONCAT('%', #{keyword}, '%')")
    List<Gorea_SearchResultTO> searchNotice(@Param("keyword") String keyword);

    // 에디터 팁 검색
    @Select("SELECT 'EditTip' AS boardType, edittipSeq AS id, edittipSubject AS title, edittipContent AS content, date_format(edittipWdate, '%Y-%m-%d') AS postDate, edittipHit AS hit FROM EditTip WHERE edittipSubject LIKE CONCAT('%', #{keyword}, '%') OR edittipContent LIKE CONCAT('%', #{keyword}, '%')")
    List<Gorea_SearchResultTO> searchEditTip(@Param("keyword") String keyword);

    // 에디터 추천 검색
    @Select("SELECT 'EditRecommend' AS boardType, editrecoSeq AS id, editrecoSubject AS title, editrecoContent AS content, date_format(editrecoWdate, '%Y-%m-%d') AS postDate, editrecoHit AS hit FROM EditRecommend WHERE editrecoSubject LIKE CONCAT('%', #{keyword}, '%') OR editrecoContent LIKE CONCAT('%', #{keyword}, '%')")
    List<Gorea_SearchResultTO> searchEditRecommend(@Param("keyword") String keyword);
    ...
    List<Gorea_SearchResultTO> searchTrendSeoul(@Param("keyword") String keyword);
    ...
    List<Gorea_SearchResultTO> searchAllBoards(@Param("keyword") String keyword);
