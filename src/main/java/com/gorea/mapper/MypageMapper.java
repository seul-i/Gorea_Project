...
public interface MypageMapper {
	...
	// 페이징
		...
		@Select("SELECT SUM(total_count) AS total_count FROM (" +
		        "SELECT COUNT(*) AS total_count FROM BestTop5 " +
		        "UNION ALL " +
	...
	// 회원 게시글 조회
		...
		@Select("SELECT '자유게시판' AS boardType, f.freeSeq AS id, f.freeTitle AS title, date_format(f.freepostDate, '%Y.%m.%d') AS postDate, f.freeHit AS hit, u.userNickname AS userNickname "
		        ...
		        + " UNION ALL "
		        + "SELECT 'BestTop5' AS boardType, b.top5Seq AS id, b.top5Comment1 AS title, date_format(b.top5postDate, '%Y.%m.%d') AS postDate, b.top5Hit AS hit, u.userNickname AS userNickname "
		        + "FROM BestTop5 b "
		        + "JOIN User u ON b.userSeq = u.userSeq "
		        + "WHERE b.userSeq=#{userSeq} LIMIT #{firstRow}, #{pageSize}")
		...
		@Select({
		    ...
		    " UNION ALL ",
		    "SELECT 'BestTop5' AS boardType, b.top5Seq AS id, b.top5Comment1 AS title, date_format(b.top5postDate, '%Y.%m.%d') AS postDate, b.top5Hit AS hit, u.userNickname AS userNickname "+ 
		    "FROM BestTop5 b JOIN User u ON b.userSeq = u.userSeq ",
		    "WHERE b.userSeq=#{userSeq}",
		    "<if test='keyword != null and keyword != \"\"'>",
		    " AND (b.top5Comment1 LIKE CONCAT('%', #{keyword}, '%'))",
		    "</if>",
		...
		// 댓글 조회
	    @Select("SELECT '트렌드 서울' AS boardType, tr.userSeq AS id, tr.seoulSeq AS boardID, tr.comment AS comment, date_format(tr.reviewDate, '%Y.%m.%d') AS postDate "
				...
				+ "UNION ALL "
				+ "SELECT '자유게시판' AS boardType, r.userSeq AS id, r.goreaboardNo AS boardID, r.replyContent AS comment, date_format(r.replypostDate, '%Y.%m.%d') AS postDate "
				+ "FROM reply r "
				+ "WHERE r.userSeq = #{userSeq} AND r.goreaboardNo = 3 "
				+ "UNION ALL "
				+ "SELECT '여행자 추천' AS boardType, r.userSeq AS id, r.goreaboardNo AS boardID, r.replyContent AS comment, date_format(r.replypostDate, '%Y.%m.%d') AS postDate "
				+ "FROM reply r "
				+ "WHERE r.userSeq = #{userSeq} AND r.goreaboardNo = 2 "
				+ "LIMIT #{firstRow}, #{pageSize}")
		...
		@Select({
		    ...
		    "</if>",
		    "UNION ALL ",
		    "SELECT '에디터 추천장소' AS boardType, err.userSeq AS id, err.editrecoSeq AS boardID, err.editRecoCmtContent AS comment, date_format(err.editrecoCmtWdate, '%Y.%m.%d') AS postDate, '' AS title ",
		    "FROM editrecommendReply err JOIN editrecommend er ON err.editrecoSeq = er.editrecoSeq ",
		    "WHERE err.userSeq = #{userSeq}",
		    "<if test='keyword != null and keyword != \"\"'>",
		    ...
		    "</if>",
		    "UNION ALL ",
		    "SELECT '자유게시판' AS boardType, r.userSeq AS id, r.goreaboardNo AS boardID, r.replyContent AS comment, date_format(r.replypostDate, '%Y.%m.%d') AS postDate, '' AS title ",
		    "FROM reply r ",
		    "WHERE r.userSeq = #{userSeq} AND r.goreaboardNo = 3 ",
		    "<if test='keyword != null and keyword != \"\"'>",
		    " AND r.replyContent LIKE CONCAT('%', #{keyword}, '%')",
		    "</if>",
		    "UNION ALL ",
		    "SELECT '여행자 추천' AS boardType, r.userSeq AS id, r.goreaboardNo AS boardID, r.replyContent AS comment, date_format(r.replypostDate, '%Y.%m.%d') AS postDate, '' AS title ",
		    "FROM reply r ",
		    "WHERE r.userSeq = #{userSeq} AND r.goreaboardNo = 2 ",
		    "<if test='keyword != null and keyword != \"\"'>",
		    " AND r.replyContent LIKE CONCAT('%', #{keyword}, '%')",
